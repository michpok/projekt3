...
def vrat_tr_tagy(vstup):
    ...
    kontrol, tagy = ziskej_trtagy(vyber_tr_tagy(odpoved))
    return tagy
...
def internacional_nebo_nacional(hodnota, obce):
    volby = []
    if 'xnumnuts' in hodnota:

        zpracovani_tagu(volby, obce)
        obvod = 0
        index = 4
        selekce = [3, 4, 7]
    else:
    ...
    return volby, obvod, index, selekce


def extraxe_jem_obci(obvod, jmena, tagy):
    if obvod == 0:
        odebrani = []
        index = 1
        klic = "overflow_name"
        nazvy_obci(index, tagy, odebrani, klic)
        for obce in odebrani:
            if 'None' not in obce:
                jmena.append(obce)
    else:
        index = 0
        klic = "s3"
        nazvy_obci(index, tagy, jmena, klic)
    return jmena


def preformatovani_csv(input_data, output):
    with open(input_data, 'r', newline='') as infile:
...
def info_hlavicka(informace, selekce, final):
    zpracovane_informace = []
    celkoveinformace = []
    ziskani_inf_hlavicka(informace, zpracovane_informace, celkoveinformace)

    # odstraneni dupliciti obalek
    selekce_informace = []
    for r in selekce:
        selekce_informace.append(zpracovane_informace[r])

    # reg. volic, obaka, platny hlas
    cisla = []
    ziskani_inf_hlavicka_zpracovani_stovky(selekce_informace, cisla)

    # pokud cislo > 999 hledam cifry tisicu
    cisla2 = []
    ziskani_inf_hlavicka_zpracovani_tisice(celkoveinformace, cisla2)

    ziskani_inf_hlavicka_zpracovani_kompletace(cisla, cisla2, final)
    return final
